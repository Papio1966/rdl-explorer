...
import { EnterpriseExtensionRepository } from "../../server/rdl/EnterpriseExtensionRepository.ts";
import { EnterpriseExtensionService } from "../../server/rdl/EnterpriseExtensionService.ts";
...
export function authenticatedExtensionContext(request:ApiRequest, context?:RequestContext) {
  ...
  if (context) logRequest("info","extension.authenticated",context,{reviewer:identity.reviewer,rateLimitRemaining:rate.remaining});
  return {identity,service:new EnterpriseExtensionService(new EnterpriseExtensionRepository(getRdlDatabaseClient())),rate};
}
