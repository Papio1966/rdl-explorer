...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  const context = beginApiRequest(request, response, "rdl-runtime.cfihos-handover-events");
  ...
  try {
    const result = await getService().handoverEvents({
    ...
    completeApiRequest(context, 200, {
      ...
      lifecycleRelationshipCount: result.lifecycleRelationshipCount,
    });
