...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  const context = beginApiRequest(request, response, "rdl-runtime.cfihos-properties");
  ...
  try {
    const result = await getService().properties({
    ...
    completeApiRequest(context, 200, {
      ...
      tagClassCount: result.tagClasses.length,
      tagClassPropertyAssignmentCount: result.tagClassPropertyAssignments.length,
      picklistValueCount: result.picklistValues.length,
    });
