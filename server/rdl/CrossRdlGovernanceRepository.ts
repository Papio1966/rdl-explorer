...
export type MappingReviewQueueItem = {
  mappingId: number; status: string; reviewVersion: number; mappingType: string; provenanceMethod: string;
  confidence: number; sourceKey: string; sourceName: string; targetKey: string; targetName: string;
  reviewedBy?: string; reviewedAt?: string; reviewRationale?: string;
};

export type MappingReviewEvent = {
  reviewEventId: number; mappingId: number; action: MappingReviewAction; fromStatus: string; toStatus: string;
  reviewer: string; rationale: string; evidence: Record<string, unknown>; reviewVersion: number; occurredAt: string;
};
  ...
  async listReviewQueue(status = "candidate", limit = 100): Promise<MappingReviewQueueItem[]> {
    ...
    const rows = await this.client.query<any>(`
      SELECT m.mapping_id,m.status,m.review_version,m.mapping_type,m.provenance_method,m.confidence,
             ss.source_key,se.name AS source_name,ts.source_key AS target_key,te.name AS target_name,
    ...
    `);
    return rows.map((row:any)=>({mappingId:Number(row.mapping_id),status:row.status,reviewVersion:Number(row.review_version),mappingType:row.mapping_type,
      provenanceMethod:row.provenance_method,confidence:Number(row.confidence),sourceKey:row.source_key,sourceName:row.source_name,targetKey:row.target_key,targetName:row.target_name,
