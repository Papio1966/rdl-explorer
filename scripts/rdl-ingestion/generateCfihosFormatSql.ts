...
import { readWorkbook, worksheetRows } from "./workbookReader.ts";
...
export async function generateCfihosFormatSql(profile: RdlWorkbookMappingProfile): Promise<string> {
  ...
  const rows = (key: string): Row[] => {
    ...
    if (!worksheet) return [];
    return worksheetRows<Row>(worksheet);
  };
