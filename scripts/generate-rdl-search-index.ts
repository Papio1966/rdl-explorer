...
type RecordOut = { sourceKey:string; sourceName:string; versionLabel:string; packageKey:string; entityType:string; nativeIdentifier:string; name:string; definition:string; sourceSheet:string };
...
function add(sourceKey:string,sourceName:string,versionLabel:string,packageKey:string,entityType:string,nativeIdentifier:string,name:string,definition:string,sourceSheet:string){if(!nativeIdentifier)return;const record={sourceKey,sourceName,versionLabel,packageKey,entityType,nativeIdentifier,name:name||nativeIdentifier,definition,sourceSheet};output.set(`${packageKey}:${entityType}:${nativeIdentifier}`,record);}
function addProfile(profile:RdlWorkbookMappingProfile){const bytes=readFileSync(profile.workbookPath);const sha=createHash("sha256").update(bytes).digest("hex");const packageKey=`${profile.sourceKey}-${profile.versionLabel.replace(/[^a-z0-9]+/gi,"-").toLowerCase()}-${sha.slice(0,12)}`;const wb=XLSX.read(bytes,{type:"buffer"});const rows=(key:string):Row[]=>XLSX.utils.sheet_to_json<Row>(wb.Sheets[profile.sheetNames[key]],{defval:null,raw:false});const t=(r:Row,f:string)=>mappedText(r,profile.fields[f]??[f]);for(const [sheetKey,type,idField,nameField,defField] of entitySpecs){for(const r of rows(sheetKey)){add(profile.sourceKey,profile.sourceName,profile.versionLabel,packageKey,type,t(r,idField),t(r,nameField),t(r,defField),profile.sheetNames[sheetKey]);}}
for(const r of rows("controlledValue")){const list=t(r,"picklistId"),value=t(r,"picklistValueCode"),seq=t(r,"picklistValueSequence");const id=t(r,"picklistValueId")||stable(profile.sourceKey,"controlled-value",[list,value,seq]);add(profile.sourceKey,profile.sourceName,profile.versionLabel,packageKey,"controlled_value",id,value||id,t(r,"picklistValueDescription"),profile.sheetNames.controlledValue);}}
...
for(const [sheet,type,idf,nf,df] of cfSpecs){for(const r of snapshot.sheets[sheet]?.rows??[]){add("cfihos","CFIHOS","2.0",cpkg,type,String(r[idf]??"").trim(),String(r[nf]??"").trim(),String(r[df]??"").trim(),sheet);}}
addProfile(CCUS_CFIHOS_FORMAT_PROFILE);addProfile(WATER_DESALINATION_PROFILE);const records=[...output.values()].sort((a,b)=>a.sourceKey.localeCompare(b.sourceKey)||a.entityType.localeCompare(b.entityType)||a.nativeIdentifier.localeCompare(b.nativeIdentifier));writeFileSync("public/rdl-search-index.json",JSON.stringify(records));console.log(`Generated ${records.length} package-aware RDL search records.`);
